...
*c_anc_any: any antenatal care visits of births in last 2 years
	gen c_anc_any = (inrange(m14,1,97))
...
*c_anc_ear: First antenatal care visit in first trimester of pregnancy of births in last 2 years
	...
	replace c_anc_ear = . if inlist(m13,98,.) 
...
*c_anc_tet: pregnant women vaccinated against tetanus for last birth in last 2 years 
	...
	*** changes 1 ***		
	...
	tab rh_anc_neotet //according to the tab infor，can gen directly
	gen c_anc_tet = rh_anc_neotet
...
*c_anc_hosp : Received antenatal care in the hospital
	...
	**** changes 2 ：another version ****
	gen c_anc_hosp_ = . 
	replace c_anc_hosp_ = 0 if !mi(m15) 
	replace c_anc_hosp_ = 1 if m57e == 1 | m57m == 1    //var 'm57e' based on antenatal care in the government hospital; var 'm57m' based on antenatal care in private hospital/clinic
    //result is the same as the original, tested in 'SierraLeone2019'/'PapuaNewGuinea2017'/'Indonesia2017'
